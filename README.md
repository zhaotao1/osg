...
**macOS**
```bash
# osgEarth 不在 Homebrew, Homebrew 只装构建工具, osgEarth 用 vcpkg
...
### macOS —— 生成 `.dmg` / `.tar.gz`
```bash
# osgEarth 走 vcpkg (Homebrew 无此 formula)
brew install cmake ninja pkg-config autoconf automake libtool
git clone https://github.com/microsoft/vcpkg ~/vcpkg && ~/vcpkg/bootstrap-vcpkg.sh
~/vcpkg/vcpkg install osgearth[core]:arm64-osx
export VCPKG_ROOT=~/vcpkg
bash scripts/pack_mac.sh
# 产物: build-mac/*.dmg  build-mac/*.tar.gz
```
- Apple Silicon 默认 **arm64**；Intel Mac 设 `VCPKG_TRIPLET=x64-osx` 并加 `-DCMAKE_OSX_ARCHITECTURES=x86_64`。
...
Windows 与 macOS 均用 **vcpkg** 装 osgEarth 并**缓存**依赖（首次慢，之后走缓存）。
