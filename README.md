...
> **推荐 conda-forge**：提供 osgEarth/OSG/GDAL/PROJ 的**预编译二进制**，一条命令装齐，
> 跨 macOS(arm64/x64)、Windows、Linux 通用，无需源码编译。

**conda-forge（推荐，全平台通用）**
```bash
conda create -n osg -c conda-forge osgearth gdal cmake ninja
conda activate osg
# 之后 CMake 用: -DCMAKE_PREFIX_PATH="$CONDA_PREFIX"  (Windows 为 %CONDA_PREFIX%\Library)
```
...
~/vcpkg/vcpkg install osgearth[core]:arm64-osx   # Intel Mac 改 x64-osx
...
> 若 CMake 找不到 osgEarth，设 `-DCMAKE_PREFIX_PATH`（conda 前缀 / osg 安装目录），
> 或环境变量 `OSGEARTH_DIR`、`OSG_DIR`。
...
### macOS —— 生成 `.dmg` / `.tar.gz`
```bash
# 推荐 conda-forge 预编译依赖
conda create -n osg -c conda-forge osgearth gdal cmake ninja
conda activate osg
bash scripts/pack_mac.sh
# 产物: build-mac/*.dmg  build-mac/*.tar.gz
```
- 脚本自动识别 `CONDA_PREFIX`（conda）或 `VCPKG_ROOT`（vcpkg）。
...
### 关于「Windows x86（32 位）」
> **不建议 32 位**：osgEarth/GDAL 的 32 位预编译库难凑齐（conda-forge 也只出 64 位），
> 且大数据渲染受 32 位地址空间（≤4GB）限制极易 OOM。生产请用 **x64**。
...
Windows 与 macOS 均用 **conda-forge 预编译二进制**（osgEarth/OSG/GDAL/PROJ 一次装齐，
无需源码编译），并**缓存 conda 环境**加速后续构建。
